...
export async function POST(req: Request) {
  try {
    ...
    try {
      ...
      if (!bucketExists) {
        console.log('Bucket "career-resumes" not found. Creating private bucket...')
        ...
        if (createBucketError) {
          ...
          console.log('Private bucket "career-resumes" created successfully.')
        }
      }
    } catch (bucketErr) {
    ...
    // 1. Upload Resume file to Supabase Storage (Private)
    ...
    // Generate Secure Signed URL for 30 days
    const { data: signedUrlData } = await supabaseServer.storage
      .from('career-resumes')
      .createSignedUrl(storagePath, 60 * 60 * 24 * 30)

    const resumeAccessUrl = signedUrlData?.signedUrl || storagePath
      ...
      <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; color: #111;">
        ...
        <table style="width: 100%; border-collapse: collapse; margin-top: 15px;">
          ...
          <tr>
            ...
            <td style="padding: 8px 0; color: #111;"><a href="${resumeAccessUrl}" target="_blank">View Resume</a></td>
          </tr>
          ${linkedinUrl ? `<tr>
